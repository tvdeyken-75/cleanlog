...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const getSessionUserKey = () => 'fahrerchecklisteSessionUser_v2';
  ...
  useEffect(() => {
    try {
      ...
      // Load active session
      const storedSessionJSON = localStorage.getItem(getSessionUserKey());
      if (storedSessionJSON) {
        const storedSession = JSON.parse(storedSessionJSON);
        setUser(storedSession.user);
        setUserRoles(storedSession.userRoles);
        setActiveRole(storedSession.activeRole);
      }
    } catch (error) {
  ...
  const login = (username: string, password: string, role: UserRole): boolean => {
    ...
    if (foundUser && foundUser.role.includes(role)) {
      try {
        const session = {
          user: foundUser.username,
          userRoles: foundUser.role,
          activeRole: role,
        };
        localStorage.setItem(getSessionUserKey(), JSON.stringify(session));
  ...
  return (
    <AuthContext.Provider value={{ isAuthenticated: !!user, login, logout, user, userRoles, activeRole, isLoading, addUser, getUsers, updateUser, deleteUser }}>
      {children}
