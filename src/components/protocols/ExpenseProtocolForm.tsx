...
export function ExpenseProtocolForm() {
  ...
  const [isReceiptDocOpen, setIsReceiptDocOpen] = useState(false);
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="container max-w-4xl mx-auto p-4 md:p-8 space-y-8">
        ...
        <Collapsible open={isReceiptDocOpen} onOpenChange={setIsReceiptDocOpen}>
            <Card>
            <CardHeader className="flex flex-row items-center justify-between cursor-pointer">
                <CardTitle className="flex items-center gap-2"><Camera className="text-primary"/>Belegdokumentation (optional)</CardTitle>
                <CollapsibleTrigger asChild>
                    <Button variant="ghost" size="sm">
                        <ChevronsUpDown className="h-4 w-4" />
                        <span className="sr-only">Toggle</span>
